// api/auth.js — Employee login (employee_id+password) AND Manager login (password only)
// Merged from login.js + manager-login.js to save a Vercel Serverless Function slot.
...
async function managerLogin(req, res) {
  ...
  const rows  = await dbQuery("SELECT value FROM settings WHERE key='manager_password'");
  const saved = rows[0]?.value || "manager@123";

  if (password !== saved)
    return res.status(401).json({ error: "Wrong manager password" });
...
module.exports = async function handler(req, res) {
  ...
  try {
    const body = req.body || {};
    // Employee login always sends employee_id; manager login never does.
    if (body.employee_id) {
