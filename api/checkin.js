// api/checkin.js — Check-in with Turso DB
...
function getIST() {
  ...
  return {
    ist, today: ist.toISOString().split("T")[0],
    h: ist.getUTCHours(), m: ist.getUTCMinutes(),
    day: ist.getUTCDay(),
    timeStr: (() => {
...
module.exports = async function handler(req, res) {
  ...
  try {
    const { telegram_user_id, location_lat, location_lng, selfie_base64 = "" } = req.body;
    ...
    const { today, h, m, day, timeStr } = getIST();

    // Already checked in?
    const existing = await dbQuery("SELECT checkin_time FROM attendance WHERE employee_id=? AND date=?", [emp.employee_id, today]);
    if (existing.length && existing[0].checkin_time) return res.status(400).json({ error: "Already checked in today" });

    // Location check
    if (location_lat && location_lng) {
      const dist = getDist(parseFloat(location_lat), parseFloat(location_lng),
                           parseFloat(s.office_lat || "28.6139"), parseFloat(s.office_lng || "77.2090"));
      if (dist > parseFloat(s.radius_meters || "100"))
        return res.status(400).json({ error: `Too far from office (${Math.round(dist)}m)` });
    }
    ...
    await dbQuery(`INSERT INTO attendance
      (employee_id,date,checkin_time,early_ot_hours,is_sunday,is_holiday,
       ot_multiplier,checkin_status,deduction_hours,location_lat,location_lng,selfie_base64,status)
      VALUES(?,?,?,?,?,?,?,?,?,?,?,?,'present')`,
      [emp.employee_id, today, timeStr, earlyOT, isSun, isHol, otMul, zone, deduct,
       location_lat, location_lng, selfie_base64.substring(0,100)]);

    if (zone === "manager_zone")
      await dbQuery("INSERT INTO manager_approvals(employee_id,date,checkin_time,decision) VALUES(?,?,?,'pending')",
                    [emp.employee_id, today, timeStr]);

    res.status(200).json({
      success: true, message: "Check-in successful!",
      employee: emp.name, employee_id: emp.employee_id,
      time: timeStr, zone, early_ot: earlyOT,
      deduction: deduct, is_sunday: !!isSun, ot_multiplier: otMul,
    });
  } catch(e) {
